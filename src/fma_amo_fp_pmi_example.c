...
/*
 * FMA AMO second generation 64 bit Floating Point test example - this test only uses PMI
 *
...
#define DOUBLE_ADD_VALUE         2.1
#define DOUBLE_MAX_VALUE         99999999999.0
#define DOUBLE_MIN_VALUE         1.1
#define LOCAL_EVENT_ID_BASE      10000000
#define MAX_NUMBER_OF_TRANSFERS  1000
#define NODE_SHIFT               10000000000
#define NUMBER_OF_TRANSFERS      80
#define REMOTE_EVENT_ID_BASE     11000000
#define TRANSFER_COUNT_SHIFT     100000
#define TRANSFER_LENGTH          1
#define TRANSFER_LENGTH_IN_BYTES ((TRANSFER_LENGTH)*sizeof(double))
...
"    second generation 64 bit floating point request.\n"
"\n"
...
"      - fma_amo_fp_pmi_example -e\n"
"      - fma_amo_fp_pmi_example -f\n"
"      - fma_amo_fp_pmi_example -f -C\n"
"      - fma_amo_fp_pmi_example -f -e\n"
"      - fma_amo_fp_pmi_example -m\n"
"      - fma_amo_fp_pmi_example -m -C\n"
"      - fma_amo_fp_pmi_example -m -I\n"
"      - fma_amo_fp_pmi_example -m -f\n"
"      - fma_amo_fp_pmi_example -m -f -C\n"
"      - fma_amo_fp_pmi_example -m -f -I\n"
"      - fma_amo_fp_pmi_example -M\n"
"      - fma_amo_fp_pmi_example -M -C\n"
"      - fma_amo_fp_pmi_example -M -I\n"
"      - fma_amo_fp_pmi_example -M -f\n"
...
{
    gni_fma_cmd_type_t amo_command = GNI_FMA_ATOMIC2_FPADD;
    ...
    while ((opt = getopt(argc, argv, "CefhImMn:v")) != -1) {
        ...
        case 'm':
            amo_command = GNI_FMA_ATOMIC2_FPMIN;

            break;

        case 'M':
            amo_command = GNI_FMA_ATOMIC2_FPMAX;
    ...
    if (use_fetch == 1) {
        switch (amo_command) {
        case GNI_FMA_ATOMIC2_FPADD:
            amo_command = GNI_FMA_ATOMIC2_FFPADD;
            break;
        case GNI_FMA_ATOMIC2_FPMAX:
            amo_command = GNI_FMA_ATOMIC2_FFPMAX;
            break;
        case GNI_FMA_ATOMIC2_FPMIN:
            amo_command = GNI_FMA_ATOMIC2_FFPMIN;
            break;
        default:
            break;
        }
    ...
    if (use_cache_request == 1) {
        ...
        switch (amo_command) {
        case GNI_FMA_ATOMIC2_FPADD:
            amo_command = GNI_FMA_ATOMIC2_FPADD_C;
            break;
        case GNI_FMA_ATOMIC2_FFPADD:
            amo_command = GNI_FMA_ATOMIC2_FFPADD_C;
            break;
        case GNI_FMA_ATOMIC2_FPMAX:
            amo_command = GNI_FMA_ATOMIC2_FPMAX_C;
            break;
        case GNI_FMA_ATOMIC2_FFPMAX:
            amo_command = GNI_FMA_ATOMIC2_FFPMAX_C;
            break;
        case GNI_FMA_ATOMIC2_FPMIN:
            amo_command = GNI_FMA_ATOMIC2_FPMIN_C;
            break;
        case GNI_FMA_ATOMIC2_FFPMIN:
            amo_command = GNI_FMA_ATOMIC2_FFPMIN_C;
            break;
        default:
            break;
        }
    ...
    my_send_to = (double) send_to * NODE_SHIFT;
    receive_from = (number_of_ranks + rank_id - 1) % number_of_ranks;
    my_receive_from = (double) receive_from * NODE_SHIFT;
    my_rank = (double) rank_id * NODE_SHIFT;
    ...
    for (i = 0; i < transfers; i++) {
        transfer_count = ((double) (i + 1.0)) * TRANSFER_COUNT_SHIFT;
    ...
    for (i = 0; i < transfers; i++) {
         ...
         */
        if ((amo_command != GNI_FMA_ATOMIC2_FPADD) &&
                   (amo_command != GNI_FMA_ATOMIC2_FFPADD)) {
        ...
        switch (amo_command) {
        case GNI_FMA_ATOMIC2_FPADD:
        case GNI_FMA_ATOMIC2_FPADD_C:
            double_operand = DOUBLE_ADD_VALUE;
            operand_ptr = (uint64_t *)&double_operand;
            fma_data_desc[i].first_operand = *operand_ptr;
            break;
        case GNI_FMA_ATOMIC2_FFPADD:
        case GNI_FMA_ATOMIC2_FFPADD_C:
            double_operand = (double) (i + 1.0) * TRANSFER_COUNT_SHIFT + DOUBLE_ADD_VALUE;
            operand_ptr = (uint64_t *)&double_operand;
            fma_data_desc[i].first_operand = *operand_ptr;
            break;
        case GNI_FMA_ATOMIC2_FPMAX:
        case GNI_FMA_ATOMIC2_FPMAX_C:
        case GNI_FMA_ATOMIC2_FFPMAX:
        case GNI_FMA_ATOMIC2_FFPMAX_C:
            if (do_invalid_compare == 0) {
                double_operand = DOUBLE_MIN_VALUE;
            } else {
                double_operand = DOUBLE_MAX_VALUE;
            }
            operand_ptr = (uint64_t *)&double_operand;
            fma_data_desc[i].first_operand = *operand_ptr;
            break;
        case GNI_FMA_ATOMIC2_FPMIN:
        case GNI_FMA_ATOMIC2_FPMIN_C:
        case GNI_FMA_ATOMIC2_FFPMIN:
        case GNI_FMA_ATOMIC2_FFPMIN_C:
            if (do_invalid_compare == 0) {
                double_operand = DOUBLE_MAX_VALUE;
            } else {
                double_operand = DOUBLE_MIN_VALUE;
    ...
    for (i = 0; i < transfers; i++) {
        ...
        case GNI_FMA_ATOMIC2_FPADD_C:
            if (i == 0) {
                ...
                expected_data =  my_rank + transfer_count +
                    ((double) (transfers * DOUBLE_ADD_VALUE));
            } else {
                expected_data =  my_rank + transfer_count;
            }
            break;
        ...
        case GNI_FMA_ATOMIC2_FFPADD_C:
            if (i == 0) {
                ...
                transfer_count = (double) (sum + 1.0) * TRANSFER_COUNT_SHIFT;
                expected_data =  my_rank + transfer_count +
                    ((double) (transfers * DOUBLE_ADD_VALUE));
            } else {
                expected_data =  my_rank + transfer_count;
        ...
        case GNI_FMA_ATOMIC2_FFPMAX_C:
            if (do_invalid_compare == 0) {
                ...
                expected_data = DOUBLE_MAX_VALUE;
            }
            break;
        case GNI_FMA_ATOMIC2_FPMIN:
        case GNI_FMA_ATOMIC2_FPMIN_C:
        ...
        case GNI_FMA_ATOMIC2_FFPMIN_C:
            if (do_invalid_compare == 0) {
                ...
                expected_data = DOUBLE_MIN_VALUE;
            }
            break;
        default:
            break;
    ...
    for (i = 0; i < transfers; i++) {
        ...
        case GNI_FMA_ATOMIC2_FPADD:
        case GNI_FMA_ATOMIC2_FPADD_C:
            transfer_count = (double) (i + 1) * TRANSFER_COUNT_SHIFT;
            expected_data = (double) (my_rank + transfer_count);
            break;
        case GNI_FMA_ATOMIC2_FFPADD:
        case GNI_FMA_ATOMIC2_FFPADD_C:
            if (i > 0) {
                transfer_count = (double) ((i) * TRANSFER_COUNT_SHIFT) + DOUBLE_ADD_VALUE;
                expected_data = expected_data + transfer_count;
            } else {
                transfer_count = (double) TRANSFER_COUNT_SHIFT;
                expected_data = (double) (my_send_to + transfer_count );
            }
            break;
        case GNI_FMA_ATOMIC2_FPMAX:
        case GNI_FMA_ATOMIC2_FPMAX_C:
