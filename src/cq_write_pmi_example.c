...
main(int argc, char **argv)
{
    ...
    rc = posix_memalign((void **) &receive_buffer, 64, TRANSFER_LENGTH_IN_BYTES);
    assert(rc == 0);
    ...
    allgather(&memory_handle, remote_memory_handle_array,
              sizeof(mdh_addr_t));
    ...
    for (i = 0; i < transfers; i++) {
        ...
        status =
            GNI_PostCqWrite(endpoint_handles_array[send_to],
                            &data_desc[i]);
        if (status != GNI_RC_SUCCESS) {
            fprintf(stdout,
